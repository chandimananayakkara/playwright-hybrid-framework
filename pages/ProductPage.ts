...
export class ProductPage {
  ...
  private productAddToCartButton: Locator;
  ...
  async selectCategory(mainMenu: string, subMenu: string) {
    let mainElement = this.page
      .getByRole("heading")
      .getByRole("link", { name: mainMenu })
      .last();
    let subElement = this.page
      .getByRole("list")
      .getByRole("link", { name: subMenu });
  ...
  async getSelectedItem(item: string) {
    ...
    await product.locator("a.add-to-cart").last().click();
  }
}
